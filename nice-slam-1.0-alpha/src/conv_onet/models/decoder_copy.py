...
class MLP(nn.Module):
    ...
    def __init__(self, name='', dim=3, c_dim=128,
        ...
        self.skips = skips
        self.bound = torch.Tensor([[0.0,6.5],[0.0,4.0],[0,3.5]])
...
class NICE(nn.Module):
    ...
    def forward(self, p, c_grid, stage='middle', **kwargs):
        ...
        if stage == 'coarse':
            ...
            occ = occ.squeeze(0)
            raw = torch.zeros(occ.shape[0], 4).float()
            raw[..., -1] = occ
            return raw
        elif stage == 'middle':
            middle_occ = self.middle_decoder(p, c_grid)
            middle_occ = middle_occ.squeeze(0)
            raw = torch.zeros(middle_occ.shape[0], 4).float()
            raw[..., -1] = middle_occ
            return raw
        elif stage == 'fine':
            fine_occ = self.fine_decoder(p, c_grid)
            raw = torch.zeros(fine_occ.shape[0], 4).float()
            middle_occ = self.middle_decoder(p, c_grid)
            middle_occ = middle_occ.squeeze(0)
